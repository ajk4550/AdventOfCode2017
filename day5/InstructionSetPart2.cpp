/*
 * Program that has a list of jumps to make, and is trying to escape from the list
 * by making a jump larger than the size of the jump. The goal is to capture the
 * number of steps it takes the computer to jump out of the data set
 * The program looks at each number to determine the size of the jump, the jump
 * can be forward or backwards. Each time you leave an index, you need to increment
 * it by 1 if the number is 2 or less. If the number is 3 or greater, decrease the
 * data by 1
 *
 * Aaron Kaye, 2017
 * Expected Answer: 28040648
 */
#include <iostream>
#include <string>
#include <sstream>
...
int main()
{
  ...
  bool escaping = true; // Indicates if we are still trying to escape
  int current = 0; // Current index that we are examining
  while(escaping) {
    steps += 1; // Increment the steps by 1
    int jump = dataArr[current]; // Amount to jump by (positive or negative)
    int currentData = dataArr[current]; // Data at the current pointer
    if(currentData > 2) {
      dataArr[current] = currentData - 1; // Decrementing by 1
    } else {
      dataArr[current] = currentData + 1; // Writing the new data (increment by 1)
    }
    ...
    if (jump > -1) {
      ...
      // Calcuate amount to move backwards
      current = current - abs(jump);
    }
    if (current > dataArr.size()-1) {
      // We are leaving the jump set
      escaping = false;
    }
  }
  // Output the final data
  cout << "This took " << steps << " steps" << endl;
  return 0;
}
