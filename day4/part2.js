/**
 * Program that evaluates a list of passwords for the number of valid
 * passwords. A password is valid if it does not have a repeating word and
 * if the words do not contain anagrams of each other.
 * Example:
...
// Running total of the number of valid passwords
let validCount2 = 0;
// The data to test against
const puzzleInput2 = `una bokpr ftz ryw nau yknf fguaczl anu
...
// Splitting the test data into an array of pass-phrases
let passPhrases2 = puzzleInput2.split('\n');

// Looping through the passwords to count the number of valid phrases
passPhrases2.forEach((pass) => {
  let words = pass.split(' '); // Splitting the passPhrase into an array of words
  let evaluating = true; // Used to indicate whether to continue the while loop
  let foundAnagram = false; // Used to indicate whether an anagram exists in the phrase
  while(evaluating) {
    let current = words.shift(); // Take the first element out of the array
    words.forEach((w) => {
      // Looping through the individuals words
      if(current == w) {
        // Word popped from array matches a word in the individual word list
        // This is not a unique password
        evaluating = false;
      }

      // Checking if the words are anagrams of each other
      let possibleAnagram = true; // Checking within a word if anagram
      let currentSorted = current.split('').sort(); // Splitting current word into letters
      let evalSorted = w.split('').sort(); // Splitting word comparing against to letters

      // Base case: Not an anagram if different lengths
      if(currentSorted.length != evalSorted.length) {
        // Can't be an anagram, different lengths
        foundAnagram = false;
      } else {
        // Comparing the two arrays. To be anagrams they should be the same
        // Assumes arrays are same size since that is checked above
        for(i=0; i<currentSorted.length; i++) {
          if(currentSorted[i] != evalSorted[i]) {
            // Found a non-match, cannot be anagram
            foundAnagram = false;
            possibleAnagram = false;
          }
        }

        // Now that you've search the whole word, update values for phrase
        foundAnagram = possibleAnagram; // Set whether an anagram was found
        if(foundAnagram) {
          // Stop evaluating, anagram is found
          evaluating = false;
        }
      }
    }); // /foreach
    if((evaluating == true) && (words.length == 0) && (foundAnagram == false)) {
      // If we get to the end of the list and have not yet found a duplicate
      // and the list does not contain anagrams
      evaluating = false; // Indiciate that the while loop should stop
      validCount2 += 1; // Increment the number of valid passwords
    }
  }
});

// Display the answer on the web page
document.querySelector("#answer2").innerHTML = `Part 2 Solution: ${validCount2}`;
