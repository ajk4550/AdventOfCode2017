 ...
 * passwords. A password is valid if it does not have a repeating word.
 * Example:
...
const puzzleInput = `una bokpr ftz ryw nau yknf fguaczl anu
...
let passPhrases = puzzleInput.split('\n');
...
passPhrases.forEach((pass) => {
  ...
  while(evaluating) {
    ...
    if((evaluating == true) && (words.length == 0)) {
      ...
      validCount += 1; // Increment the number of valid passwords
    }
  }
});

// Display the answer on the web page
document.querySelector("#answer1").innerHTML = `Part 1 Solution: ${validCount}`;
