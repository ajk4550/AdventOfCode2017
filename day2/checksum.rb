# Checksum Program - Advent Day 2
# This program calculates the checksum of the provided data
# Aaron Kaye, 2017
# Expected Answer: 54426
...
# Splitting the data into row-arrays
row_arr = data.split("\n")
# Looping through the row arrays for the individual elements
row_arr.each do |row|
  # Splitting into individual elements
  el_arr = row.split("\t")
  min = 4611686018427387903 # Integer MAX
  max = 0
  # Looping through the elements arrays for the largest and smallest data
  # in that row
  el_arr.each do |x|
    x = x.to_i
    if x > max
      # x is now the max element visited
      max = x
    end
    if x < min
      # x is now the min element visited
      min = x
    end
  end
  # Cacluate the difference
  diff = max - min
  # Store the difference in a running sum
  sum += diff
  # Reset max and min
  max = 0
  min = 0
