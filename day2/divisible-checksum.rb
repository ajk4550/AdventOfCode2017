...
# This program calculates the checksum of the provided data by summing division
# result from the  two elements in each row that are divisible
# Aaron Kaye, 2017
# Expected Answer: 333
...
row_arr.each do |row|
  ...
  el_arr = row.split("\t")
  # Looping through the elements arrays for the divisible elements in that row
  quotient = 0
  el_arr.each_with_index do |n, i|
    n = n.to_i
    el_arr.each_with_index do |x, j|
      if i != j
        x = x.to_i
        if n % x == 0
          quotient = n / x
  ...
  # Store the quotient in a running sum
  sum += quotient
  # Reset quotient
  quotient = 0
end
